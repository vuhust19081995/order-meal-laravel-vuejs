...
const formData = defineModel<TMealOrderForm>(
    'formData',
    {required: true}
)
const formRef = ref();
...
function onChangeDish(index, id) {
  formData.value.dishes[index].name = props.dishesForSelection?.find((item) => item.id === id)?.name ?? ''
}
...
<template>
  ...
  <a-form ref="formRef" :model="formData">
    <a-row v-for="(selectedDish, index) in formData.dishes">
      <a-col :span="12">
        <a-form-item :name="['dishes', index, 'id']" :rules="[{ required: true, message: 'Please choose a dish' }]">
          <a-select
      ...
      <a-col :span="12">
        <a-form-item :name="['dishes', index, 'quantity']"
                     :rules="[{ required: true, message: 'This field is required' }]">
          <a-input-number type="number" v-model:value="selectedDish.quantity" :min="1" :max="10"/>
        </a-form-item>
      </a-col>
    </a-row>
    <a-button @click="addMoreDish()" type="primary" shape="circle">
      +
    </a-button>
